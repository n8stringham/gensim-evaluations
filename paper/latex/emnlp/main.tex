...
%A number of other intrinsic methods exist such as measuring relatedness of word pairs and their correlation to human judgments of semantic similarity \cite{schnabel2015evaluation}. 
%But these methods are limited by the high overhead cost required to pick appropriate word pairs 
%and the experiments required to obtain human relatedness scores. 
%This lack of scalability restricts authors to evaluate on only a small number of popular languages. 
...
%We tune these models using $\OddOneOut$ and $\topk$  and introduce conduct a qualitative downstream task that investigates how religious words are used in each language.
...
%    \resizebox{\columnwidth}{4.25in}{\includegraphics{cross_cat_acc-horizontal.pdf} }
\resizebox{\columnwidth}{5in}{\includegraphics{cross_cat_acc-horizontal.pdf} }
\caption{
...
%    \resizebox{2.3in}{2.3in}{\includegraphics{emoji_exp.pdf} }
    \resizebox{\columnwidth}{\columnwidth}{\includegraphics{emoji_exp.pdf} }
\caption{
...
%\resizebox{.9\textwidth}{1.5in}{
\begin{tabular}{cccc}
&& &\multicolumn{1}{c}{\hspace{5em} Wikidata Category} \\ 
\rotatebox[origin=c]{90}{Language}& \multicolumn{3}{c}{\raisebox{-.5\height}{\includegraphics{lct-biblical.pdf}}} \\
\end{tabular}
%}

\caption{ 
A qualitative investigation of religious topics across the different language models. For the most part, results match our intuition regarding which topics are relevant to each language.
%\vspace{-.1in}
%  \fixme{}
%    The LCT task allows qualitative investigation into the topics most discussed in the texts our models were trained on. 
    %(\textbf{Top})
...
%Though Europe was heavily Christianized at the times all these languages were spoken,
...
%This highlights the importance of selecting topics with a level of specificity appropriate to the analysis.
\end{enumerate}
...
In this paper we introduce the first word vector evaluation methods designed specifically for the low-resource domain, $\OddOneOut$ and $\topk$,
along with a method for automatically producing test sets in Wikidata's 581 supported languages.
We believe Wikidata is an underutilized resource in the NLP evaluation community,
and in particular, that its massively multilingual support can be used to better serve under resourced languages.
